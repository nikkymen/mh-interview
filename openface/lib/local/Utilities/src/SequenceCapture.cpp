...
void SequenceCapture::CaptureThread()
{
    ...
    while(capturing)
    {
        ...
        if (!is_image_seq)
        {
            ...
            int target_frame = (int)std::round(actual_frame_count * skip_ratio);

            // Skip frames if needed
            while (frame_num_int <= target_frame)
            {
            ...
            if (capturing)
            {
                bool success = capture.retrieve(tmp_frame);
            ...
            timestamp_curr = actual_frame_count * (1.0 / desired_fps);
        }
